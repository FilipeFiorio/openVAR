...
class OpenVAR:
    ...
    def iniciar_checagem(self):
        ...
        # Reduz ruído mantendo as bordas nítidas
        ...
        # Executa a inferência de Pose do YOLO11x
        self.jogadores = self.pose_detector.detect_players(imagem_upscale)
        
        # Converte a imagem de BGR para RGB para o Tkinter/PIL
    ...
    def jogador_no_clique(self, rx, ry):
        """Determina qual jogador foi selecionado pelo clique do mouse."""
    ...
    def clique_esquerdo(self, event):
        ...
        # Fase de calibração ativa se houver menos de 4 cliques
        if len(self.pontos_calibracao) < 4:
    ...
    def clique_direito(self, event):
        ...
        # Fase de marcação de defensores
        did = self.jogador_no_clique(rx, ry)
        if did is None:
            return
    ...
    def redesenhar(self):
        ...
        # Desenha as caixas dos jogadores
        for j in self.jogadores:
            x1, y1, x2, y2 = j["bbox"]
            sx1 = int(x1 * self.escala)
            sy1 = int(y1 * self.escala)
            ...
            # Desenha linha amarela na perna (pé)
        ...
        # Desenha pontos de calibração clicados
        ...
        # Conecta os pontos de calibração
    ...
    def calcular_e_desenhar(self):
        ...
        for j in self.jogadores:
            if j["id"] in self.time_defensor or j["id"] in self.time_atacante:
                ...
                # Atualiza dinamicamente pe_x e pe_y para o pé selecionado
                j["pe_x"] = pe_x
                j["pe_y"] = pe_y
        ...
        img_rgb = cv2.cvtColor(self.imagem_com_pose, cv2.COLOR_RGB2BGR) # volta pra BGR pra manter consistência
        ...
        # Desenha as Bounding Boxes e marcas dos jogadores
        for j in self.jogadores:
            did = j["id"]
    ...
    def mostrar_resultado(self, img_pil, impedido):
        ...
        # Colore o veredito separadamente
